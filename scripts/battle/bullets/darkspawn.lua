...
function DarkSpace:init(x, y, sprite)
    ...
    self.damage = 18
    self.collider = CircleCollider(self, 0, 0, 15)
...
function DarkSpace:update()
    ...
    if (self.dist < self.inaccurate_distance_calculation_variable + 8) then
        ...
        if love.math.random(1, 3) == 1 then
            local particle = Game.battle:addChild(Registry.createObject("titan_generic_particle",
                self.x + Utils.random(-12, 12), self.y + Utils.random(-12, 12)))
