...
local MAX_DISTANCE  = 85   -- GM room-scale half-gap distance; scaled down to Kristal's arena size below
local SCALE         = 0.75 -- Kristal arenas run smaller than GM's box -- scale MAX_DISTANCE down to match
...
local EXTRA_OPEN_HOLD = 15 -- linger open for half a second before closing, so delayed bullets are dodgeable
...
local DIFF = {
    [0] = { spawn_speed = 48, slashes = 5, hold = 30, damage = 31, max_dist = 85, bullets = 13 },
    [1] = { spawn_speed = 44, slashes = 6, hold = 30, damage = 31, max_dist = 85, bullets = 13 },
    [2] = { spawn_speed = 40, slashes = 6, hold = 26, damage = 31, max_dist = 85, bullets = 13 },
    [3] = { spawn_speed = 39, slashes = 5, hold = 30, damage = 31, max_dist = 85, bullets = 13 },
    [5] = { spawn_speed = 70, slashes = 5, hold = 50, damage = 31, max_dist = 120, bullets = 14 },
}
...
function KnightBoxSplit:computeTime()
    ...
    -- Each slash now owns the whole indicator -> split -> close cycle. Budget enough time for every
    -- requested slash plus a short tail instead of relying on overlapping manager timers.
    local cycle_frames = IND_TIME + SPLIT_WAIT + d.hold + (self.hold_open or EXTRA_OPEN_HOLD) + 3
    self.time = (slashes * cycle_frames + 30) / 30
end
...
function KnightBoxSplit:onStart()
    ...
    -- Manager state: each slash winds up its rotating indicator for 30 frames, tears the box, then
    -- waits for the halves to snap fully shut before the next indicator begins.
    local d = DIFF[self.difficulty] or DIFF[3]
...
function KnightBoxSplit:_spawnToothBullet(x, y, dir, top_speed, accel, damage, delayed)
    self:spawnBullet("knight_split_bullet", x, y, dir, top_speed, accel, damage)

    if not delayed then
        Game.battle.timer:after(0.5, function()
            if self.parent and not self.finished then
                self:_spawnToothBullet(x, y, dir, top_speed * 1.08, accel, damage, true)
            end
        end)
    end
end

-- The cut lands after the previous split has completely settled. Keeping one complete cycle per slash
-- prevents a fresh indicator from deleting decorative halves while they are still opening or closing.
...
function KnightBoxSplit:_spawnSplitBullets()
    ...
    if self.difficulty == 5 then
        ...
        for wave = 0, 1 do
            ...
            for i = 0, count - 1 do
                ...
                self:_spawnToothBullet(self.cx, by, flip and 180 or 0, spd, -0.18, self.split_damage)
    ...
    for i = 0, BULLET_COUNT - 1 do
        ...
        if diagonal then
            ...
            self:_spawnToothBullet(self.cx, self.cy, dir, top_speed, accel, self.split_damage)
        else
            local sx, sy = self.cx + seam_x * pos, self.cy + seam_y * pos
            self:_spawnToothBullet(sx, sy, flip and perp_deg_a or perp_deg_b, top_speed, accel, self.split_damage)
        end
...
function KnightBoxSplit:update()
    ...
    -- Charge the manager clock while a split is active, but only spend it when the box is fully intact.
    -- This preserves the quick rhythm (the next telegraph begins right after the snap) without allowing
    -- a slash to cut off an opening/closing animation in progress.
    if self.slash_count < self.n_slashes then
        self.spawn_timer = math.min(self.spawn_speed, self.spawn_timer + DTMULT)
        if self.con == 0 and not self.ind_active and self.spawn_timer >= self.spawn_speed then
            self.spawn_timer = 0
            self:_startSlash()
        end
    end
...
function KnightBoxSplit:_updateSplitCycle()
    ...
    elseif self.con == 2 then
        ...
        if self.timer >= hold / 2 then
            ...
            if (self.hold_open or EXTRA_OPEN_HOLD) > 0 then
    ...
    elseif self.con == 2.5 then
        ...
        if self.timer >= (self.hold_open or EXTRA_OPEN_HOLD) then
            self.con, self.timer = 3, 0
        end
