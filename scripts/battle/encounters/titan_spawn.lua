local TitanSpawnEncounter, super = Class(Encounter, "titan_spawn")

function TitanSpawnEncounter:init()
    super.init(self)
    ...
    local desperate = self:addEnemy("titan_spawn", 520, 130)
    table.remove(desperate.waves, 2)

    local evolving = self:addEnemy("titan_spawn", 520, 290)
...
function TitanSpawnEncounter:onTurnEnd()
    self.difficulty = self.difficulty + 1
end

function TitanSpawnEncounter:beforeStateChange(old, new)
    if self.toggle_smoke then
        if old == "INTRO" and not self.darkness_controller then
            Game.battle.timer:after(1, function()
                if not Game.battle or Game.battle.encounter ~= self then return end
                self.darkness_controller = Game.battle:addChild(Registry.createObject("titan_darkness_controller"))
...
function TitanSpawnEncounter:onStateChange(old, new)
