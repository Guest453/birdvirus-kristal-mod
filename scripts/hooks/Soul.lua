...
function Soul:init(x, y, color)
    ...
    self.radius = 0
    self.toggle = false
    self.radius_goal = Game.battle and Game.battle.encounter and Game.battle.encounter.light_size or 48
end


function Soul:draw()
    if self.toggle then
    self.radius_goal = Game.battle and Game.battle.encounter and Game.battle.encounter.light_size
        or self.radius_goal or 48
